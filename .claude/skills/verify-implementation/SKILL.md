...
| # | 스킬 | 설명 |
|---|------|------|
| 1 | `verify-api-routes` | API Route 에러 처리, 입력 검증, 데모 폴백, 한국어 에러 메시지, HTTP 상태 코드 검증 |
| 2 | `verify-dashboard-pages` | 대시보드/인증 페이지 클라이언트 선언, 한국어 UI, shadcn/ui 사용, 로딩/에러 상태 처리 검증 |
| 3 | `verify-landing-pages` | 랜딩 페이지 한국어 마케팅 텍스트, 가격 정보 일관성, shadcn/ui 사용, 아이콘 통일 검증 |
| 4 | `verify-plan-limits` | 통합 크레딧 시스템(CREDIT_COSTS/PLAN_CREDITS/FREE_ALLOWED_FEATURES) 및 checkCredits/deductCredits 일관 적용 검증 |
| 5 | `verify-shared-utils` | 공유 유틸리티 중복 코드 재발 방지, import 경로 일관성, 미사용 export 검증 |
| 6 | `verify-seo-engine` | SEO 점수 체계(100점 만점) 및 등급 판정의 엔진/API/UI 일관성 검증 |
| 7 | `verify-security` | 봇 차단/Rate Limiting/robots.txt/보안 헤더 3레이어 일관성 검증 |
| 8 | `verify-ai-provider` | AI 제공자(Gemini/Claude) 라우팅, Free 플랜 Gemini 강제, 키워드 Gemini 고정 검증 |
| 9 | `verify-admin-system` | 관리자 인증(verifyAdmin), 시스템 설정/사용자 관리 API, 관리자 UI 일관성 검증 |
| 10 | `verify-blog-learning` | 블로그 학습 파이프라인 수집 경로 연동, 프롬프트 주입, barrel export, admin client 사용 일관성 검증 |
